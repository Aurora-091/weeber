...
|---|---|---|
| Section 3 — customer confirms | `confirmCodOrder({ shop, orderId, confirmed: true, notes })` | Tags the order `cod-confirmed` in Shopify via weebersh (`orders/annotate`) — see `packages/api/src/voice/tools/confirmCodOrder.ts` |
| Section 3 — customer cancels | `confirmCodOrder({ shop, orderId, confirmed: false, notes })` | **Cancels the order immediately** via weebersh (`orders/cancel`, `reason: "DECLINED"`) — resolved (was previously flagged here as an open issue: an explicit "no" used to only get recorded and wait for 3 failed/declined attempts via the workflow engine's `onExhausted` hook before actually cancelling, same as a no-answer would. That's wrong for an *explicit* decline — real information from the customer, not an unknown outcome to retry into — so it now short-circuits straight to cancellation instead of going through the retry-then-give-up path). The retry/exhaustion path (`DECISIONS.md` ADR-030) still applies to genuine no-answer/busy/failed outcomes, unchanged. |
