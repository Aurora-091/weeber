...
**Correction (2026-07-29):** The workflow itself is *not* missing — `.github/workflows/ci.yml` exists and is well-built: parallel `typecheck` / `test` / `build` / `lint` / `migration-drift` / `e2e` jobs, plus an `if: always()` `ci-success` aggregator job that fails if any dependency failed. The workflow header comment already documents the intended setup. The test scripts are also correct: `packages/api` and `packages/web` both run `bun test --isolate src/`, and `packages/openvent-compliance` (bare `bun test`) has no `mock.module` leakage (verified 63 pass / 0 fail bare). So there is **no false-red risk** in CI and no code change needed. The only gap is that branch protection isn't wired to require it.

**Recommendation (revised):** Require the single **`ci-success`** status check on `main` in branch protection (Settings → Branches → Require status checks to pass before merging), and enable "Require a pull request before merging" so direct pushes can't bypass it. Do **not** require the individual jobs — the workflow header explicitly warns against this, since `ci-success` is the stable aggregator that never needs re-configuring when jobs are added/renamed/split. This is a repo-settings change, not code — it's the fix that prevents findings #1 and #4 from recurring.
...
| 3 | CI not gating merges | 🟡 **Repo-settings, user-only.** Workflow already exists and is correct (`ci.yml`, with a `ci-success` aggregator; api/web tests use `--isolate`, compliance verified clean bare — no false-red risk). Only gap: require the single **`ci-success`** check on `main` in GitHub branch protection + "Require a pull request before merging". Do NOT require individual jobs (workflow header warns against it). Not a code change. `main` is green now, so safe to turn on immediately. |
| 5 | Type-escape hatches | ℹ️ **Swept 2026-07-29 — no action needed.** Read every product (non-test) `as any`/`as never` (22 total). All benign: provider adapters (`salesforce`/`hubspot`/`gohighlevel`/`google-calendar`) cast `(await res.json()) as any` at the external-API JSON boundary (standard untyped-response pattern); `server.ts`'s 5 `ws as never` casts are each guarded by a `ws.data.kind` discriminant check immediately before, so they're type-safe workarounds for Bun's `ServerWebSocket` handler typing not narrowing on the union; `stream.ts` `[] as never[]` is an empty-result catch fallback; `error-handler.ts` `err.statusCode as any` is `number`-vs-Hono-`StatusCode`-union friction. **None hide a real type hole or mask a bug.** Not refactored — churning a P3 the audit judged healthy isn't worth the risk. |
| 6 | `format.ts` partial adoption | ✅ **Done 2026-07-29.** Migrated all 9 admin dashboard pages (`workflow-runs`, `waitlist`, `users`, `support`, `settings`, `orgs`, `compliance`, `calls-list`, `broadcasts`) off inline `new Date(...).toLocaleString(...)` onto `lib/format.ts` (`formatDateTime`, and `formatDate` for `orgs`). Local `formatWhen`/`formatDate` wrapper names kept so call sites are untouched; `"never"` null-guards preserved where they were meaningful (settings/orgs/compliance). **Behavior note:** admin timestamps now include the year and use `format.ts`'s `en-US` default (was browser-locale, no year) — this deliberately unifies admin rendering with the app pages, which already use `formatDateTime`. **Exception:** `logs.tsx` keeps its own formatter because it needs second-level precision, which `format.ts`'s scannable datetime variants intentionally omit — left as-is rather than fragment the shared API for one caller. Number formatting (`billing.tsx`/`home.tsx` `.toLocaleString()`) was out of scope (not date/time). Verified: web typecheck clean, lint 0/0, 16/16 web tests pass. |
