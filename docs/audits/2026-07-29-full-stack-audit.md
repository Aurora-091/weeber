...
> **Update 2026-07-29 (post-fix):** The two red-gate findings (#1 orphan test, #4 lint autofocus) are **fixed and verified** — `typecheck`, `lint`, and `test` are all green again (662 pass / 0 fail). See *Resolution status* at the bottom. Findings #2 (telephony plaintext) and #3 (CI enforcement) remain open by design — see their notes. The body below is preserved as the as-found record.
...
| 2 | **P1 (High, carryover)** | Security / data-at-rest | Telephony auth tokens (`orgs.twilioAuthToken`) still dual-written & read as **plaintext columns** despite the Vault existing. Unchanged since 2026-07-19 finding #1 (first half); CRM/calendar half was fixed, this half was not. | 🔴 Open (needs go-ahead) |
...
| 2 | Telephony auth tokens plaintext (carryover) | 🔴 **Open by design.** Security-sensitive cutover: requires routing `twilioAuthToken` reads/writes through `store_org_credential`/`read_org_credential`, then a follow-up migration to drop the plaintext `orgs` columns once the read path is switched. Not started — held for explicit go-ahead. Still the oldest open security item; do it before onboarding a BYO-telephony paying merchant. |
