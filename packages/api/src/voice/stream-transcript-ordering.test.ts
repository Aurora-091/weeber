...
const dbLike = {
  ...
  insert: (table: unknown) => {
    ...
    return chain([{ id: dbInserts.length + 1 }], { onValues: (values) => dbInserts.push({ table: name, values }) });
  },
  update: () => chain([]),
  execute: async () => [],
};
...
describe("B4 — transcripts.sequence reflects real conversational order", () => {
  it("assigns strictly increasing sequence values across the greeting and two full turns", async () => {
    ...
    stt.getLastOnTranscript()?.({ text: "turn one caller line", isFinal: true, speechFinal: true });
    await flush();
    stt.getLastOnTranscript()?.({ text: "turn two caller line", isFinal: true, speechFinal: true });
    await flush();
