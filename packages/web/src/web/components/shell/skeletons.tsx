...
export function SkeletonTable({ columns = 4, rows = 6 }: { columns?: number; rows?: number }) {
  return (
    <div className="overflow-hidden rounded-lg border border-border" style={{ boxShadow: "var(--weeber-shadow-card)" }} aria-hidden>
  ...
  return (
    <div className="grid gap-[var(--shell-card-gap)] sm:grid-cols-2 lg:grid-cols-3" aria-hidden>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-lg border border-border bg-card p-4" style={{ boxShadow: "var(--weeber-shadow-card)" }}>
